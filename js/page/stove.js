H.stove = {
	...
	setStealUin: function(uin) {
		...
		this.showStoveTree(this._selected_theme_id);
	},
	showStoveTree: function(themeId, reload) {
	...
	showThemes: function() {
		...
		this._my_theme_array.length == 0 ? div.append('<strong class="tips_title">您当前没有正在合成的卡片</strong>') : div.append('<strong>正在合成的卡：</strong>');
		for (var len = this._my_theme_array.length, i = 0, obj; i < len; i++) {
			obj = this._my_theme_array[i];
			div.append('<a id="' + obj.tid + '" href="javascript:void(1);" onclick="H.stove.init();H.stove.showStoveTree(' + obj.tid + ');">' + H.ui.getThemeMiniLogo(obj.tid) + '</a>');
		}
		div.append('<button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="H.stove.showSelectTheme();" title="选择主题"><span class="ui-button-text">选择主题</span></button>');
	},
	showSelectTheme: function() {
		H.ui.showSelectTheme({
			handleTheme: function(tid) {
				H.stove.init();
				H.stove.showStoveTree(tid);
			}
		});
	},
	initThemeMap: function(themeId) {
		...
		function getNeedCardNumMap(cardId, flag) {
			if (!H.stove._need_num_map[cardId]) H.stove._need_num_map[cardId] = 0;
			H.stove._need_num_map[cardId] += flag;
	...
	showTheme: function(themeId) {
		jQuery("#my_composing_themes a").each(function(index, domEle) {
			if (parseInt(jQuery(domEle).attr("id")) == themeId) {
				jQuery(domEle).addClass("current");
			} else {
				jQuery(domEle).removeClass("current");
			}
		});
		var div = jQuery("#theme_tree");
		if (div.children().length > 0) {
			div.empty();
		}
		...
		function getComposeButton(o) {
			if (CARD.data.mapCompose[o[0]]) {
				...
				var meny = o[3];
				if (H.stove._my_box_card_map[CARD.data.mapCompose[o[0]][3]]) flag1 = true;
				if (H.stove._my_box_card_map[CARD.data.mapCompose[o[0]][4]]) flag2 = true;
				if (H.stove._my_box_card_map[CARD.data.mapCompose[o[0]][5]]) flag3 = true;
				...
				can_steal = H.stove._my_stove_num_map.steal_empty > 0 ? true : false;
				can_compose = H.stove._my_stove_num_map.home_empty > 0 ? true : false;
				var html = '';
				html += '<div class="width_100 text_align_center clear">';
				if (H.stove._steal_uin > 0) {
					if (meny == 40) {
						if (can_steal) {
							if (direct_compose) {
								html += '<button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove.submitCompose(' + themeId + ',' + o[0] + ');" title="偷"><span class="ui-button-text">偷</span></button>';
							} else {
								html += '<button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove.submitCompose(' + themeId + ',' + o[0] + ');" title="买偷"><span class="ui-button-text">买偷</span></button>';
							}
						} else {
							if (direct_compose) {
					...
					if (can_buy_compose) {
						if (can_compose) {
							if (direct_compose) {
								html += '<button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove.submitCompose(' + themeId + ',' + o[0] + ');" title="合成"><span class="ui-button-text">合成</span></button>';
							} else {
								html += '<button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove.submitCompose(' + themeId + ',' + o[0] + ');" title="购合"><span class="ui-button-text">购合</span></button>';
							}
						} else {
							if (direct_compose) {
						...
						if (can_compose) {
							if (direct_compose) {
								...
								html += '<button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove.submitCompose(' + themeId + ',' + o[0] + ');" title="自动"><span class="ui-button-text">自动</span></button>';
							}
						} else {
							if (direct_compose) {
								html += '<button type="button" class="ui-button ui-widget ui-state-active ui-corner-all ui-button-text-only" onclick="javascript:void(0);" title="合成"><span class="ui-button-text">合成</span></button>';
		...
		var html = '<div class="level"><button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.submitCollection.init(' + themeId + ');" title="已集齐"><span class="ui-button-text">已集齐</span></button></div>';
		for (var j = 0; j < this._my_theme_map._time_temp_arr.length; j++) {
			...
			html += '<div class="level"><p class="info"><strong class="meny"><span>面值 </span>' + CARD.data.mapCard[this._my_theme_map._time_arr[this._my_theme_map._time_temp_arr[j]][0]][3] + '</strong><strong class="time"><span>耗时 </span>' + hour + ':' + min + ':00</strong></p>';
			...
			for (i = 0, len = this._my_theme_map._time_arr[this._my_theme_map._time_temp_arr[j]].length; i < len; i++) {
				...
				html += '<li id="' + card[0] + '" title="' + card[2] + '" onmouseout="javascript:H.stove.mouseOutComposeItem(' + card[0] + ')" onmouseover="javascript:H.stove.selectItem(this,' + card[0] + ',false)" onclick="javascript:H.stove.selectItem(this,' + card[0] + ',true);" class="card_mini float_left text_align_center">' + '<div class="card_mini_img width_100">' + H.ui.getCardMiniImg(card[0]) + getStat(card[0]) + '</div>' + getComposeButton(card) + '</li>';
			}
			html += '</ul></div>';
		}
		html += '<div class="level">';
		if (H.checkRP(themeId)) {
			html += '<p class="info"><strong class="meny">免合成套卡只能通过抽卡和变卡获得！</strong></p>';
		} else if (H.checkFlashCard(themeId)) {
			html += '<p class="info"><strong class="meny">闪卡素材卡通过变卡获得，详情参考 <a target="_blank"href="http://service.qq.com/info/59576.html">如何炼闪卡</a></strong></p>';
		} else if (H.checkJbCard(themeId)) {
			var str = (!H.checkCanTransfer(themeId) ? '' : '变卡或');
			html += '<p class="info"><strong class="meny">已下架绝版卡无法购买，只能' + str + '与卡友交换！</strong></p>';
		} else {
			html += '<p class="info"><strong class="meny">素材卡</strong><strong class="time">售价：20金币/张</strong></p>';
		}
		html += '<ul class="overflow_auto">';
		for (var i = 0, len = this._my_theme_map._normal_card_arr.length; i < len; i++) {
			...
			if (!H.checkRP(themeId) && !H.checkFlashCard(themeId) && !H.checkJbCard(themeId)) {
				html += '<div class="width_100 text_align_center clear"><button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove.buy(' + card[0] + ',function(){H.stove.showStoveTree(' + card[1] + ')});" title="购买"><span class="ui-button-text">购买</span></button></div></li>';
			} else if (H.checkFlashCard(themeId)) {
				html += '<div class="width_100 text_align_center clear"><button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove._dst_trans_card_id=' + card[1] + ';H.stove.buyFlashCard(' + card[1] + ',' + card[0] + ');" title="购变"><span class="ui-button-text">购变</span></button></div></li>';
			}
		}
		if (H.checkRP(themeId)) {} else if (H.checkFlashCard(themeId)) {
			html += '<li class="float_left"><button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove.oneKeyBuyAndTransfer();" title="一键购变"><span class="ui-button-text">一键购变</span></button></li>';
		} else if (H.checkJbCard(themeId)) {} else {
			html += '<li class="float_left"><button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" onclick="javascript:H.stove.oneKeyBuy();" title="一键购买"><span class="ui-button-text">一键购买</span></button></li>';
		}
		html += '</ul></div></div>';
	...
	oneKeyBuyAndTransfer: function() {
		for (var i = 0, len = this._my_theme_map._normal_card_arr.length; i < len; i++) {
			var card = CARD.data.mapCard[this._my_theme_map._normal_card_arr[i]];
			H.stove._dst_trans_card_id = card[0];
			H.stove.buyFlashCard(card[1], card[0]);
		}
	},
	oneKeyBuy: function() {
		for (var i = 0, len = this._my_theme_map._normal_card_arr.length; i < len; i++) {
			var card = CARD.data.mapCard[this._my_theme_map._normal_card_arr[i]];
			H.stove.buy(card[0], function() {
				H.stove.showStoveTree(card[1]);
			});
		}
	},
	selectItem: function(o, id, selected) {
		...
		jQuery("#theme_tree .card_mini.mouse_on").removeClass("mouse_on");
		jQuery("#theme_tree .card_mini.linked").removeClass("linked");
		var d = CARD.data.mapCompose[id];
		if (!d) return;
		jQuery("#theme_tree .card_mini#" + id).addClass("mouse_on");
		for (var i = 0; i < 3; i++) {
			jQuery("#theme_tree .card_mini#" + d[i + 3]).addClass("linked");
		}
	},
	...
	buyCard: function(themeId, cardId) {
		...
		if (CARD.data.mapTheme[themeId][16] == 0) {
			this.buy(cardId, function() {
				H.stove.showStoveTree(themeId);
				if (H.stove._dst_card_id) H.stove.submitCompose(themeId, H.stove._dst_card_id);
			});
		} else {
	...
	buyFlashCard: function(themeId, cardId) {
		...
		for (var i = 0, len = normalCarArr.length; i < len; i++) {
			...
			if (this._my_box_card_map[srcCardId] > 0) {
				...
				setTimeout(function() {
					H.stove.transCard(cardId, srcCardId, slotId, slotType, function() {
						H.stove.showStoveTree(themeId);
						if (H.stove._dst_card_id) H.stove.submitCompose(themeId, H.stove._dst_card_id);
						if (H.stove._dst_trans_card_id) H.stove.buyFlashCard(themeId, cardId);
					});
				}, 2000);
				return;
			}
		...
		this.buy(srcCardId, function() {
			...
			setTimeout(function() {
				H.stove.transCard(cardId, srcCardId, slotId, 0, function() {
					H.stove.showStoveTree(themeId);
					if (H.stove._dst_card_id) H.stove.submitCompose(themeId, H.stove._dst_card_id);
					if (H.stove._dst_trans_card_id) H.stove.buyFlashCard(themeId, cardId);
				});
	...
	submitStealStove: function(o, fnSucceed) {
		...
		function callback_onOk(iVeryCode) {
			H.ui.waitStart();
			var sUrl = 'http://card.show.qq.com/cgi-bin/card_stove_steal?ver=1&opuin=' + H.stove._steal_uin + '&slot1=' + o.slot1 + '&slot2=' + o.slot2 + '&slot3=' + o.slot3 + '&slottype1=' + o.slottype1 + '&slottype2=' + o.slottype2 + '&slottype3=' + o.slottype3 + '&themeid=' + o.themeid + '&targetid=' + o.targetid + '&targettype=' + o.targettype + '&code=' + iVeryCode;
		...
		function fnError(iCode, sMsg, iEndTime) {
			...
			if (iCode == -1001) {
				...
				H.ui.showErrDlg({
					title: H.stove._steal_uin > 0 ? '偷炉失败' : '炼卡失败',
					msg: sMsg,
