define(['HBase', 'HUser'], function(HBase, HUser) {
	return {
		getThemeMiniLogo: function(themeId) {
			var theme = CARD.data.mapTheme[themeId];
			if (!theme) return '<img/>';
			return '<img src="' + HBase.getExtensionBaseUrl() + 'images/theme/' + themeId + '_logo" title="' + theme[1] + '"/>';
		},
		getThemeBigLogo: function(themeId) {
			var theme = CARD.data.mapTheme[themeId];
			if (!theme) return '<img/>';
			return '<img src="' + HBase.getExtensionBaseUrl() + 'images/theme/' + themeId + '_big_logo" title="' + theme[1] + '"/>';
		},
		getCardMiniImg: function(cardId) {
			var card = CARD.data.mapCard[cardId];
			if (!card) return '<img/>';
			return '<img title="' + card[2].escHtml() + '" src="' + HBase.getExtensionBaseUrl() + 'images/card/' + card[0] + '_56" />';
		},
		getCardBigImg: function(cardId) {
			var card = CARD.data.mapCard[cardId];
			if (!card) return '<img/>';
			return '<img title="' + card[2].escHtml() + '" src="' + HBase.getExtensionBaseUrl() + 'images/card/' + card[0] + '" />';
		},
		getCardBigImgSrc: function(cardId) {
			return HBase.getExtensionBaseUrl() + 'images/card/' + cardId;
		},
		getThemeImgSrc: function(themeId) {
			return HBase.getExtensionBaseUrl() + 'images/theme/' + themeId;
		},
		getImgItemWithBg: function(cardId) {
			var card = CARD.data.mapCard[cardId];
			if (!card) return '';
			var html = '';
			html += '<img class="position_absolute theme" src="' + HBase.getExtensionBaseUrl() + 'images/theme/' + card[1] + '" />';
			html += '<img class="position_absolute card" title="' + card[2] + '" src="' + HBase.getExtensionBaseUrl() + 'images/card/' + card[0] + '"/>';
			html += '<span class="position_absolute name" style="color:' + this.getThemeFontColor(card[1]) + ';"><strong>' + card[2] + '</strong></span>';
			html += '<span class="position_absolute meny" style="color:' + this.getThemeFontColor(card[1]) + ';"><font style="padding-right:9px;">' + card[3] + '</font></span>';
		...
		showBox: function(arg) {
			...
			slotArr.sort(HBase.sortCardFunc);
			var html = '';
		...
		showSelectTheme: function(arg) {
			...
			for (var themeId in CARD.data.mapTheme) {
				if (HBase.checkFlashCard(themeId)) {
					arg.FlashTotal++;
				} else if (HBase.checkJbCard(themeId)) {
					arg.JbTotal++;
				} else {
					arg.NorTotal++;
				}
			}
			if (HUser.bGotCollection) {
				showSelectThemeDetail(arg);
			} else {
				waitStart();
				HUser.loadMyColletion(function() {
					showSelectThemeDetail(arg);
					waitEnd();
				})
			}
		...
		showSelectThemeDetail: function(arg) {
			...
			for (var themeId in HUser.mapCollection) {
				if (HBase.checkFlashCard(themeId)) {
					arg.FlashNum++;
				} else if (HBase.checkJbCard(themeId)) {
					arg.JbNum++;
			...
			for (var themeId in CARD.data.mapTheme) {
				var theme = CARD.data.mapTheme[themeId];
				if (themeId == HBase.props.PROPS_THEME_ID) {
					continue;
				}
				var index = 0;
				if (HBase.checkJbCard(themeId) != 1) {
					index = 0;
				} else if (HBase.checkJbCard(themeId) && !HBase.checkFlashCard(themeId)) {
					index = 1;
				} else if (HBase.checkFlashCard(themeId)) {
					index = 2;
				}
			...
			for (var i = 0; i < tempArr.length; i++) {
				...
				for (var j = 0; j < tempArr[i].length; j++) {
					...
					for (var k = 0; k < tempArr[i][j].length; k++) {
						...
						html += '                <li onclick="javascript:HUi.handleTheme(' + theme[0] + ')" class="float_left text_align_center width_23';
						var num = 0;
						if (HUser.mapCollection[theme[0]] && HUser.mapCollection[theme[0]].num) {
							num = HUser.mapCollection[theme[0]].num;
						}
						if (num > 0) {
							html += ' collected';
						}
