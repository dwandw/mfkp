require.config({
	paths: {
		...
		"HBase": "page2/base",
		"HUser": "page2/user",
		"HStoveBox": "page2/stove_box"
	}
});
define(['jQueryUi', 'HBase', 'HUser', 'HStoveBox'], function(_, HBase, HUser, HStoveBox) {
	return {
		injectDiv: function() {
			if (CARD.checkLogin()) {
				var html = '';
				...
				html += '            <button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" onclick="javascript:HUser.load();"><span class="ui-button-text">更新</span></button>&nbsp;&nbsp;&nbsp;&nbsp;';
				...
				HBase.init();
				HUser.load(function() {
					jQuery('#SPAN_NICK').html(HUser.getOMyData().nick + '(' + HUser.getOMyData().uin + ')');
					HStoveBox.showStoveBox();
